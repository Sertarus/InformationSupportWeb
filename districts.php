...
if (isset($_POST['addButton'])) {
        	session_start();
        	header("location: districts_create_edit.php?isEdit=0");
        }
...
?>
 <!DOCTYPE html>
  ...
  <head>
    ...
    <script src="js/districts.js?2"></script>
    <style type="text/css">
      body {
  ...
  <body>
    <nav class="navbar navbar-dark bg-dark" aria-label="First navbar example">
      <div class="container-fluid">
        <a class="navbar-brand" href="districts.php">Районы</a>
    ...
    <form method="post">
                <?php if ($_SESSION["role"] == 2) echo "<input type='submit' class='btn btn-outline-primary' value='Добавить район' style='float: right;' name='addButton'><input type='submit' class='btn btn-outline-danger' value='Удалить объекты с пометкой на удаление' style='float: left;' name='deleteButton'>"; ?>
            </form>
    <table id="districts_table" class="table table-bordered table-hover">
      <?php
        require_once "config.php";
        $sql = "select name, d.createdby, TO_CHAR(d.creationdate, 'DD.MM.YYYY HH24:MI:SS') as creationdate, u.login as creator_login from districts d join users u on u.iduser = d.createdby where d.deleted = '0'";
        ...
        "</table>";
            ...
            echo "<tbody>".
        "<tr>".
        "<td>На данный момент не создано ни одного района</td>".
        "</tr>".
        "</tbody>".
        "</table>";
        }
