...
if (isset($_POST['addButton'])) {
        	session_start();
        	header("location: events_create_edit.php?isEdit=0");
        }
...
?>
 <!DOCTYPE html>
  ...
  <head>
    ...
    <script src="js/events.js?2"></script>
    <style type="text/css">
      body {
  ...
  <body>
    <nav class="navbar navbar-dark bg-dark" aria-label="First navbar example">
      <div class="container-fluid">
        <a class="navbar-brand" href="events.php">Мероприятия</a>
    ...
    <form method="post">
                <input type="submit" class="btn btn-outline-primary" value="Добавить мероприятие" style="float: right;" name="addButton">
    ...
    <table id="objects_table" class="table table-bordered table-hover">
      <?php
        require_once "config.php";
        $sql = "select idevent, name, TO_CHAR(timestart, 'DD.MM.YYYY HH24:MI:SS') as timestart, TO_CHAR(timeend, 'DD.MM.YYYY HH24:MI:SS') as timeend, e.createdby, TO_CHAR(e.creationdate, 'DD.MM.YYYY HH24:MI:SS') as creationdate, u.login as creator_login from events e join users u on u.iduser = e.createdby where e.deleted = '0'";
        ...
        if (oci_execute($stmt)) {
            if (oci_fetch($stmt)) {
              ...
              $serv_sql = "select * from events_services where event = :p1 and service = :p2 and deleted = '0'";
        ...
        "</table>";
            ...
            echo "<tbody>".
        "<tr>".
        "<td>На данный момент не создано ни одного мероприятия</td>".
        "</tr>".
        "</tbody>".
        "</table>";
        }
