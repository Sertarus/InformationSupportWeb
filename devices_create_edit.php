...
if (isset($_POST['addButton'])) {
          ...
          if (strlen($_POST['name']) >= 1 && strlen($_POST['name']) <= 40) {
            ...
            $sql = "select * from devices where name = :p1 and deleted = '0'";
            if ($stmt = oci_parse($link, $sql)) {
              oci_bind_by_name($stmt, ':p1', $name);
              if (oci_execute($stmt)) {
                if (oci_fetch($stmt)) {
                  $name_err = "Устройство с таким именем уже существует";
                  oci_free_statement($stmt);
                  oci_close($link);
                }
                else {
                  if (filter_var($_GET["isEdit"], FILTER_VALIDATE_BOOLEAN)) {
                    $sql = "update devices set name = :p1, changedby = :p2, changeddate = SYSTIMESTAMP where name = :p3";
                    if ($stmt = oci_parse($link, $sql)) {
                      oci_bind_by_name($stmt, ':p1', $name);
                    ...
                    $sql = "insert into devices (name, createdby, creationdate) values (:p1, :p2, SYSTIMESTAMP)";
                    if ($stmt = oci_parse($link, $sql)) {
                      oci_bind_by_name($stmt, ':p1', $name);
                      oci_bind_by_name($stmt, ':p2', $_SESSION["iduser"]);
                      if (!oci_execute($stmt)) {
  ...
  <body>
    <nav class="navbar navbar-dark bg-dark" aria-label="First navbar example">
      <div class="container-fluid">
        <a class="navbar-brand" href="devices.php">Назад</a>
    ...
    if (filter_var($_GET["isEdit"], FILTER_VALIDATE_BOOLEAN)) {
      echo "<form method='post'><div class='form-group w-25 mx-auto m-4'>
        ...
        <span class='help-block'>". $name_err . "</span>
                ...
                <input type='submit' class='btn btn-primary' value='Изменить устройство' name='addButton'>
            </form>";
    }
    else {
      echo "<form method='post'><div class='form-group w-25 mx-auto m-4'>
