...
$sql = "update dataobjects set  changeddate = SYSTIMESTAMP, changedby = :p1, deleted = 1 where name = :p2 and deleted = 0";
...
$sql = "update recordvalues set changedby = :p1, changeddate = SYSTIMESTAMP, deleted = 1 where deleted = 0 and dataobject in (select iddataobject from dataobjects where name = :p2)";
if ($stmt = oci_parse($link, $sql)) {
	oci_bind_by_name($stmt, ':p1', $_SESSION["iduser"]);
	oci_bind_by_name($stmt, ':p2', $_GET["name"]);
	if (!oci_execute($stmt)) {
...
if ($stmt = oci_parse($link, $sql)) {
      ...
      if (oci_execute($stmt)) {
        while (oci_fetch($stmt)) {
          $rec_upd_sql = "update recordtypes set deleted = '1', changedby = :p1, changeddate = SYSTIMESTAMP where name = :p2 and deleted = 0";
